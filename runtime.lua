...
local displays = {}
local last_image = {}
local DeviceSelect = {} -- hash lookup instead of iterating through Controls.DeviceSelect e.g. DeviceSelect['Reception 1'] exists if it is selected in a Control.DeviceSelect combo
...
function SetDevicePower(i, device, command) -- command == 'Power_on', 
  ...
  print('display "'..Properties['Display Code Name Prefix'].Value..i..'" type('..type(display_)..')') -- serdata
  ...
  if display_ then -- a display module with the command is connected to the display so use it for power
    if command=='Display_off' then 
      ...
      if display_['PanelOff']~=nil then 
        command_ = 'PanelOff'
      elseif display_['ScreenOff']~=nil then 
        command_ = 'ScreenOff'
    ...
    elseif command=='Display_on' then 
      ...
      elseif display_['ScreenOn']~=nil then 
        command_ = 'ScreenOn'
        if display_['PowerStatus']~=nil and not display_['PowerStatus'].Boolean then 
          command2_ = 'PowerOn'
        end
...
function UpdateDisplayModule(i, device) -- device is a single device
  ...
  if displays[i] and type(displays[i])~="boolean" then
    ...
    if displays[i]['IPAddress'] then
      if DebugDisplays then print("display["..i.."] type: ".. device['type']) end
      if (device['platform_name'] == nil or device['platform_name']=='') and device['type']=='sssfp5Lfd' then
          if DebugDisplays then print("display["..i.."] IPAddress: ".. device['ip']) end
          displays[i]['IPAddress'].String = device['ip'] -- system on chip (an app running on the display), update the display module ip address
        else
          if displays[i]['IPAddress'].String 
           and displays[i]['IPAddress'].String ~= '' 
           and not string.match(displays[i]['IPAddress'].String, 'Address')
          then -- can't be blank or default 'Enter IP Address'
          if DebugDisplays then print("device.ip: "..displays[i]['IPAddress'].String) end
...
function UpdateDevice(i, device) -- device is a single device
  if device==nil then
    ...
    DeviceSelect[device['name']] = device
    local display_ = UpdateDisplayModule(i, device)
    if DebugFunction then print('UpdateDisplayModule returned '..type(display_)) end
    ...
    --CheckContent(device)
    if DebugFunction then print('trying to update channel from device') end
    local status_ = {}
    if type(device.content) == "table" then
...
function UpdateDeviceData(i) -- i is the index in devices[i]
  if DebugFunction and DeviceSelect[devices[i].name] then print('UpdateDeviceData('..i..') "'..devices[i].name..'"') end
...
function load_control_event_handlers()
  for i=1, Properties['Display Count'].Value do
    ...
    Controls['PowerOn'][i].EventHandler = function(ctl) -- power_on
      if DebugDisplays then print('PowerOn['..i..'] event: '..tostring(ctl.Boolean)) end
     local _, device_ = GetDeviceData(i)
      if device_~=nil then SetDevicePower(i, device_, 'Display_on') end
    end
    
    Controls['PowerToggle'][i].EventHandler = function(ctl) -- power_on
      if DebugDisplays then print('PowerToggle['..i..'] event: '..tostring(ctl.Boolean)) end    
...
function ParseDevice(device)  -- data is a single device
  if DebugFunction and DeviceSelect[device['name']] then print('device data response: '..device['name']) end
  local found_ = false
  for i=1, #devices do --each device
    if devices[i]['id'] == device['id'] then
      if not helper.equals(devices[i], device[i], false) then
        if DebugFunction and DeviceSelect[device['name']] then print('Updating data in device ['..i..']: '.. devices[i]['name']) end
...
function ReportStatus(state,msg)
  if status ~= StatusState[state] then --if Controls.Status.Value ~= StatusState[state] then
	  if DebugFunction then -- Controls.Status.Value doesn't update correctly so using a variable
      print("ReportStatus("..state..") "..(msg or ''))
    end
	end
  local msg = msg or ""
...
function ResponseHandler(tbl, code, data, err, headers)
  ...
  elseif code == 403.0 or Controls.IPAddress.String == "" then  -- Invalid Address handler
...
-- EventHandlers
-----------------------------------------------------------------------------------------------------------------------
local function load_setup_event_handlers()
...
if not Controls.Disable or not Controls.Disable.Boolean then initialize() end
-----------------------------------------------------------------------------------------------------------------------
-- End of module
-----------------------------------------------------------------------------------------------------------------------